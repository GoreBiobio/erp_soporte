...
class GenerarWord extends Controller
{
    ...
    public function generar_word_comodato(Request $request)
    {
        ...
        for ($r = 1; $r <= 1; $r++) {
            $table->addRow();
            for ($c = 1; $c <= 1; $c++) {
                $table->addCell(500)->addImage('http://sitio.gorebiobio.cl/wp-content/themes/html5blank/img/auxi/logo-header-200x300.png', array(
                        'width' => 60,
                        'height' => 80,
                        'marginTop' => -1,
                        'marginLeft' => -1,
        ...
        if ($comodatos->contratoFunc == 1) {
            ...
            $text = "En Concepción, con fecha " . date("d", strtotime($comodatos->fechaIngComod)) . " de " . date("m", strtotime($comodatos->fechaIngComod)) . " de " . date("Y", strtotime($comodatos->fechaIngComod)) . ", se procede a realizar la entrega a título de comodato al" .
                " Sr(a). " . strtoupper($comodatos->nombresFunc) . " " . strtoupper($comodatos->paternoFunc) . " " . strtoupper($comodatos->maternoFunc) . ", Consejero(a) Regional del Gobierno Regional, del siguiente objeto:";
...
•	DAF.";
        ...
        $escribir->save(storage_path($comodatos->idComod . '_COMODATO.docx'));

        return response()->download(storage_path($comodatos->idComod . '_COMODATO.docx'));

    }


    public function generar_word_devolucion(Request $request)
    {
        $id_com = $request->input('idCom');
        ...
        if ($comodatos->contratoFunc == 1) {
            ...
            $text = "En Concepción, con fecha" . date("d", strtotime($comodatos->fechaDevolComod)) . " de " . date("m", strtotime($comodatos->fechaDevolComod)) . " de " . date("Y", strtotime($comodatos->fechaDevolComod)) . ", se procede a realizar la recepción a " . strtoupper($comodatos->nombresFunc) . " " . strtoupper($comodatos->paternoFunc) . " " . strtoupper($comodatos->maternoFunc) . ", Funcionario(a) del Gobierno Regional, del siguiente objeto:";
            ...
            $text = "En Concepción, con fecha " . date("d", strtotime($comodatos->fechaDevolComod)) . " de " . date("m", strtotime($comodatos->fechaDevolComod)) . " de " . date("Y", strtotime($comodatos->fechaDevolComod)) . ", se procede a realizar la recepción a título de comodato al" .
                " Sr(a). " . strtoupper($comodatos->nombresFunc) . " " . strtoupper($comodatos->paternoFunc) . " " . strtoupper($comodatos->maternoFunc) . ", Consejero(a) Regional del Gobierno Regional, del siguiente objeto:";
        ...
        if ($comodatos->contratoFunc == 1) {
            ...
            $text = "Se deja constancia que el presente comodato se efectúa en pro del comodante conforme a lo dispuesto en el Art.2179" .
                " del Código Civil; en atención a lo dispuesto en el Art.43 bis de la Ley N°19.175, ya que es necesidad del Servicio" .
                " mantener una comunicación expedita con el Sr(a). Consejero(a) Regional para los efectos de citaciones a sesiones de Consejo.";
        ...
        $text4 = strtoupper($comodatos->name) . "<w:br /><w:br />RECIBE <w:br />PROFESIONAL GOBIERNO REGIONAL";
...
•	DAF.";
        ...
        $escribir = \PhpOffice\PhpWord\IOFactory::createWriter($word, 'Word2007');
        $escribir->save(storage_path($comodatos->idComod . '_FINALIZA_COMODATO.docx'));

        return response()->download(storage_path($comodatos->idComod . '_FINALIZA_COMODATO.docx'));
    }
